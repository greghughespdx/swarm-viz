...
| HUD overlay | Live counts: agents, messages, tokens, cost |
| Cost panel | Per-agent cost breakdown with model used |
| Work items | Active agents and their current bead/task |
| Activity particles | Tool calls rippling outward from agent orbs |

Agent capabilities have distinct colors: coordinator, lead, scout, builder, reviewer, merger.

### Keyboard Shortcuts

All toggles are also clickable in the lower-right HUD.

| Key | Action |
|-----|--------|
| `L` | Toggle agent name labels |
| `M` | Toggle message labels on arc trails |
| `G` | Toggle force-directed clustering |
| `W` | Toggle work items panel (upper-left) |
| `C` | Cycle cost display: both > labels only > panel only > off |
...
The server auto-discovers Overstory projects by scanning sibling directories for `.overstory/` folders. It reads state databases in **read-only** mode and pushes incremental updates over WebSocket. When a project has active agents, the server switches to live mode automatically. The browser renders a live force-directed graph using Three.js with UnrealBloom post-processing.
...
| `metrics.db` | No | Token usage and cost per agent session |

If `metrics.db` is absent the visualization runs normally; cost and token metrics show as zero.
...
swarm-viz/
├── client/
│   ├── index.html       # Shell HTML with HUD elements and CSS
│   ├── main.ts          # Entry: WebSocket + scene wiring + HUD logic
│   ├── dashboard.ts     # Dashboard overlay (mode banner, transitions)
│   ├── scene.ts         # Three.js scene, force layout, animations
│   └── websocket.ts     # Reconnecting WebSocket client
├── server/
│   ├── index.ts         # Bun server: SQLite polling, WebSocket push, auto-discovery
│   └── mappers.ts       # DB row and metrics transformations
├── shared/
